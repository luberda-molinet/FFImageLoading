...
{
    internal class ImageCache : IMemoryCache<NSImage>
    {
        readonly NSCache _cache;
        static IMemoryCache<NSImage> _instance;
        ...
        public static IMemoryCache<NSImage> Instance
        {
            get
            {
		...
		public Tuple<NSImage, ImageInformation> Get(string key)
        {
            if (string.IsNullOrWhiteSpace(key))
                return null;

			var image = (NSImage)_cache.ObjectForKey(new NSString(key));
			...
			return new Tuple<NSImage, ImageInformation>(image, imageInformation);
        }

		public void Add(string key, ImageInformation imageInformation, NSImage value)
        {
